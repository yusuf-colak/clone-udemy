...
async function uploadWithProgress(
  ...
  const fileName =
    tenant + '/' + (folder ? folder + '/' : '') + (customName ?? file.name);
  ...
  return new Promise((resolve, reject) => {
    reader.onload = async (event: any) => {
      ...
      try {
        ...
        resolve({ file, fileName, status: 'uploaded', url }); // Resolve the promise with fileName
      } catch (err: any) {
...
async function resizeAndUploadThumbnail(
  tenant: string,
  folder: string | null,
  file: any,
  idx: number,
