...
export class TrackingsController {
  ...
  @checkAbilites({ action: 'manage', subject: 'Tracking' })
  @UseGuards(JwtAuthGuard, AbilitiesGuard)
  @Get('user/:userId/courseId/:courseId/openChapterId')
  findNextChapterId(
    @Param('userId') userId: string,
    @Param('courseId') courseId: string
  ) {
    return this.trackingsService.findNextChapterId(userId, courseId);
  }
  //-----
  ...
  async createFind(@Body() data: CreateTrackingDto) {
